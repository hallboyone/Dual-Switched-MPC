...
A11 = {eye(2) + 0.25*(rand(2)-0.5), eye(2) + 0.25*(rand(2)-0.5)};
A22 = {eye(2) + 0.25*(rand(2)-0.5), eye(2) + 0.25*(rand(2)-0.5)};
...
B1 = {[0; 0.1] + 0.1*(rand(2,1)-0.5), [0.1; 0] + 0.1*(rand(2,1)-0.5)};
B2 = {[0; 0.1] + 0.1*(rand(2,1)-0.5), [0.1; 0] + 0.1*(rand(2,1)-0.5)};
%B = {B1, B2};
B = saved_vars.B_good;
...
while true
    ...
    old_S = S;
    parfor a=1:2
        S_new = NodalSafeSets(system(a), W{a});
        W{a} = convexHullOfUnion(S_new);
    end
    parfor a=1:2
        S_new = NodalSafeSets(system(a), W{a});
        W{a} = convexHullOfUnion(S_new);
    ...
    if ~isempty(old_S) && areEqual(S{1}, S{2}, true)
        break
    end
end
