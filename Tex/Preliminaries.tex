...
An external switching signal is any discrete function, $\ss(\cdot)$ that is uninfluenced by the system's state and control inputs and causes elements of the plant and/or controller to switch. In this work, it is assumed that the signal respects some graph, $\ss(\cdot)\in\Sigma(\graph)$ that is known at design time. The state of the switching signal can then be defined by the label of the current node, $\ssl(t)\triangleq \gnodelabel{\ss(t)}$. Prior knowledge of the switching signal is not required, but its value at time $t$ is known immediately.
\begin{remark}
